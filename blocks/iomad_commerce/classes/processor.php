...
class processor {
    ...
    public static function licenseblock_onordercomplete($invoiceitem, $invoice) {
        ...
        try {
            // Use the company from the invoice first (correct company on gateway/bank approvals);
            // fall back to the current user's company.
            ...
            if (!empty($paths)) {
                ...
                foreach ($courses as $course) {
                    ...
                    // Ensure the course is associated with the company so its managers can see it.
                    if (!$DB->record_exists('company_course', ['companyid' => $companyid, 'courseid' => $course->courseid])) {
                        $DB->insert_record('company_course', ['companyid' => $companyid,
                                                              'courseid' => $course->courseid,
    ...
    private static function singlepurchase_onordercomplete($invoiceitem, $invoice) {
        ...
        try {
            ...
            if (!empty($paths) || $licensecoursecount > 0) {
                ...
                // Get the company id from the invoice first (correct company on gateway/bank approvals);
                // fall back to the current user's company.
                if (!empty($invoice->companyid)) {
                    $companyid = $invoice->companyid;
                } else {
